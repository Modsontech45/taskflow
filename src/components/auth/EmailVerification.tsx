...
export function EmailVerification() {
  ...
  const verifyEmail = async (token: string) => {
    try {
      ...
      showToast('error', 'Verification failed', err.message || 'Please try again or contact support.');
    }
  };

  const getIcon = () => {
    switch (status) {
      case 'verifying': return <Mail className="w-16 h-16 text-blue-500 animate-pulse" />;
      case 'success': return <CheckCircle className="w-16 h-16 text-green-500" />;
      case 'error': return <XCircle className="w-16 h-16 text-red-500" />;
    }
  };
  ...
  const getTitle = () => {
    switch (status) {
      ...
      case 'success': return 'Email verified successfully!';
          ...
          <div className="space-y-3">
            ...
            {status === 'error' && (
              <div className="space-y-3">
                ...
                <Link to="/login" className="block text-sm text-blue-600 hover:text-blue-800 transition-colors">
